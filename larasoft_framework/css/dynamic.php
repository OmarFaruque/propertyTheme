...
function dynamic_css(){
	...
	if(!empty($get_option['main_color'])){
		$css .= '.bg-extra-blue, 
				div#maps a{background-color:'. hex2rgba($get_option['main_color'], 1).';}';
		$css .= '.footerCopyright{background-color:'. hex2rgba($get_option['main_color'], 0.4).';}';
		$css .= '.fituredMeatas{background-color:'. hex2rgba($get_option['main_color'], 0.8).';}';
		$css .= 'section#register:before{background-color:'. hex2rgba($get_option['main_color'], 0.6).';}';
		$css .= 'ul#menu-top ul.sub-menu li a:before, ul#menu-top ul.sub-menu li a:hover:after{background-color:'. hex2rgba($get_option['main_color'], 0.1).';}';
		
	}
	if($get_option['layout_color_style'] && $get_option['layout_color_style'] == 'light'){
		$css .= '.bbutton a, .panel.panel-default.panel-blue{border-color:'.hex2rgba($get_option['main_color'], 0.5).'}';
		$css .= '.panel-blue .panel-footer.bg-dip-blue, 
			input.bg-dip-blue, 
			.bg-dip-blue,
			.btn, .btn-drk-blue,
			.headerSocial li.social a:hover,
			.tabDiv .tabItems ul li a:hover, 
			.tabDiv .tabItems ul li a:focus, 
			.tabDiv .tabItems ul li.active a,
			form#homeSearchForm2 button, 
			.testmBody,
			form#homeSearchForm button,
			.bbutton a:hover
			
			{ background-color: '. hex2rgba($get_option['main_color'], 0.5) .' !important; }

			.headerSocial li.social a, 
			.welcomeMessage h2,
			.colr-blue,
			.singleSidebar .menu li a:hover,
			nav#mainNav ul li a:hover,
			article.logoAddressSocial address:before,
			.bbutton a,

			#homeSearchForm2 label, #homeSearchForm label
			{color:'.hex2rgba($get_option['main_color'], 1).'}
		';
	}else{
	
		$css .= '.testmBody{background-color:'. hex2rgba($get_option['main_color'], 0.5).';}';
		$css .= '.panel-blue .panel-footer.bg-dip-blue, 
			input.bg-dip-blue, 
			.bg-dip-blue,
			.btn, .btn-drk-blue,
			...
			{ background-color: '. hex2rgba($get_option['main_color'], 1) .' }
